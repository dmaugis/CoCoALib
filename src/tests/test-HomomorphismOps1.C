...
namespace CoCoA
{
  ...
  void program()
  {
    ...
    RingHom phi =PolyAlgebraHom(R,S, RImage1); 
    ...
    RingHom psi = PolyAlgebraHom(R,S, RImage2);
...
//   phi3: QQ[x,y] --> QQ[x,y]/<y+1>,  x |--> (x), y |--> (y)
     ...
     RingHom phi4 = PolyAlgebraHom(P1, Q2, images);

     // phi4: QQ[x,y,z] --> QQ[a,b]/<a^2+4b^2-1> sending  x |--> (2b)   y |--> (0)   z |--> (a)
     CoCoA_ASSERT_ALWAYS(!IsInjective(phi4));
     CoCoA_ASSERT_ALWAYS(ker(phi4) == ideal(RingElem(P1, "y"), RingElem(P1, "x^2+z^2-1")));
