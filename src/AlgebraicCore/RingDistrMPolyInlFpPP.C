...
// ***ATTENTION*** [20140726] is it worth having this special case code???
  ...
  {
    CoCoA_ERROR(ERR::ShouldNeverGetHere, "RingDistrMPolyInlFpPPImpl::myRecvTwinFloat");
  }
...
// Special case code; should be a bit more efficient that general case -- is it really worth it???
  ...
  SparsePolyRing NewPolyRing_DMPII(const ring& CoeffRing, long NumIndets)
  {
    ...
    return NewPolyRing_DMPII(CoeffRing, IndetNames, StdDegRevLex(NumIndets));
  }
  ...
  SparsePolyRing NewPolyRing_DMPII(const ring& CoeffRing, long NumIndets, const PPOrderingCtor& OrdCtor)
  {
    ...
    const vector<symbol> IndetNames = SymbolRange("x", 0, NumIndets-1);
    return NewPolyRing_DMPII(CoeffRing, IndetNames, OrdCtor(NumIndets));
  }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/RingDistrMPolyInlFpPP.C,v 1.47 2018/06/13 15:07:07 abbott Exp $
