...
//    }
  ...
  void MoveLMToFront(DistrMPolyInlFpPP& f, DistrMPolyInlFpPP& g)
  {
    ...
    if (g.mySummands == 0/*nullptr*/) g.myEnd = &(g.mySummands);
    ltg->myNext = 0/*nullptr*/; // not really needed, but checked when debugging is active
    f.myPushFront(ltg);
  }
...
//      clog << "------------------------------------------------------"<<std::endl;
  ...
  {                                                 //???
    if (IsZero(h)) CoCoA_ERROR(ERR::NotNonZero, "DistrMPolyInlFpPP::myAddMul");
...
//   }
  ...
  {
    CoCoA_ERROR(ERR::ShouldNeverGetHere, "DistrMPolyInlFpPP::ReductionStepGCD");
  }
...
//   }
  ...
  void DistrMPolyInlFpPP::myPushBack(summand* t)
  {
    ...
    if (!IsZero(*this))
    {
      ...
      while (LastSummand->myNext != 0/*nullptr*/)
  ...
  void DistrMPolyInlFpPP::myAddMonomial(const DistrMPolyInlFpPP& g)
  {
    ...
    if (f_smnd == 0) { myPushBack(grab(NewTerm)); return; }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/DistrMPolyInlFpPP.C,v 1.32 2018/05/22 14:16:39 abbott Exp $
