...
//    RingDistrMPolyInlPPImpl(const ring& R, const std::vector<symbol>& IndetNames, const PPOrdering& ord);
  ...
  public:  // functions which every ring must implement
    ...
    virtual std::string myImplDetails() const {return "RingDistrMPolyInlPP";}
    ...
    virtual void myMulByCoeff(RawPtr rawf, ConstRawPtr rawc) const; ///< EXCEPTION SAFE
    virtual bool myDivByCoeff(RawPtr rawf, ConstRawPtr rawc) const; ///< EXCEPTION SAFE
    //@}
  ...
  ConstRefRingElem RingDistrMPolyInlPPImpl::myZero() const
  {
    return *myZeroPtr;
  }


  ConstRefRingElem RingDistrMPolyInlPPImpl::myOne() const
  {
    return *myOnePtr;
  }
  ...
  long RingDistrMPolyInlPPImpl::myNumIndets() const
  {
    return myNumIndetsValue;
  }


  const ring& RingDistrMPolyInlPPImpl::myCoeffRing() const
  {
    return myCoeffRingValue;
  }


  const std::vector<RingElem>& RingDistrMPolyInlPPImpl::myIndets() const
  {
  ...
  const PPMonoid& RingDistrMPolyInlPPImpl::myPPM() const
  {
    return myPPMValue;
  }
...
void RingDistrMPolyInlPPImpl::myReductionStep(RawPtr rawf, ConstRawPtr rawg) const
  ...
  SparsePolyRing NewPolyRing_DMPI(const ring& CoeffRing, const std::vector<symbol>& IndetNames, const PPOrdering& ord)
  {
    if (!IsCommutative(CoeffRing))
      CoCoA_ERROR(ERR::NotCommutative, "NewPolyRing_DMPI pseudo ctor");
    if (NumIndets(ord) != len(IndetNames))
      CoCoA_ERROR(ERR::BadNumIndets, "NewPolyRing_DMPI pseudo ctor");
    if (!AreGoodIndetNames(CoeffRing, IndetNames))
      CoCoA_ERROR(ERR::BadIndetNames, "NewPolyRing_DMPI pseudo ctor");
  ...
  {
    if (NumIndets <= 0)
      CoCoA_ERROR(ERR::BadNumIndets, "NewPolyRing_DMPI(CoeffRing, NumIndets)");
  ...
  {
    if (NumIndets <= 0)
      CoCoA_ERROR(ERR::BadNumIndets, "NewPolyRing_DMPI(CoeffRing, NumIndets, ord)");
  ...
  {
    if (!IsPPMonoidOv(PPM))
      CoCoA_ERROR(ERR::BadArg, "NewPolyRing_DMPI(k, PPM) -- PPM is not of Ov type");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/RingDistrMPolyInlPP.C,v 1.46 2018/06/13 15:07:08 abbott Exp $
