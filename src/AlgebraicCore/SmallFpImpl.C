...
namespace CoCoA
{
  ...
  inline SmallFpImpl::repr_t SmallFpImpl::ourCalcHalfwayPoint(repr_t p)
  {
    CoCoA_ASSERT(p >= 2);
  ...
  inline long SmallFpImpl::ourCalcIterLimit(repr_t p)
  {
    CoCoA_ASSERT(p >= 2);
    const repr_t MAX = numeric_limits<repr_t>::max();
  ...
  SmallFpImpl::SmallFpImpl(const MachineInt& n, GlobalSettings::ResidueSetting ResidueChoice):
  ...
  SmallFpImpl::SmallFpImpl(SmallPrime p, GlobalSettings::ResidueSetting ResidueChoice):
  ...
  bool SmallFpImpl::IsGoodCtorArg(SmallPrime p)
  {
  ...
  SmallFpImpl::value SmallFpImpl::myReduce(const MachineInt& n) const
  {
  ...
  SmallFpImpl::value SmallFpImpl::myReduce(const BigInt& N) const
  {
  ...
  SmallFpImpl::value SmallFpImpl::myPower(value x, long n) const
  {
    CoCoA_ASSERT(x == myNormalize(x));
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SmallFpImpl.C,v 1.38 2018/06/25 12:28:21 abbott Exp $
// $Log: SmallFpImpl.C,v $
