  ...
  namespace Involutive {
    ...
    void UniversalInvolutiveBasisContainer::myInitializeJBMill() const
    {
      if (!jbMillPtr.get())
      {
        ...
        jbMillPtr = auto_ptr<JBMill>(new JBMill(builder));
      }
    }
    ...
    void UniversalInvolutiveBasisContainer::myInitializeAndValidatePBMill(string fcnName) const
    {
      if (!pbMillPtr.get())
      {
        ...
        if (!IamDeltaRegular())
        {
          CoCoA_ERROR("This ideal is not delta regular", fcnName);
        }
        ...
        pbMillPtr = auto_ptr<PBMill>(new PBMill(converter));
      }
    }
    ...
    void UniversalInvolutiveBasisContainer::myValidateHomogeneous(string fcnName) const
    {
      if (!IamHomogeneous())
      {
        CoCoA_ERROR("This ideal is not homogeneous", fcnName);
      }
    }
    ...
    void UniversalInvolutiveBasisContainer::myValidateDegRevLexOrdering(string fcnName) const
    {
      ...
      {
        CoCoA_ERROR(ERR::PPOrder, fcnName);
      }
    }
    ...
    void UniversalInvolutiveBasisContainer::myValidateMonomialIdeal(string fcnName) const
    {
      if (!IamMonomial())
      {
        CoCoA_ERROR("This ideal is not monomial", fcnName);
      }
    }
    ...
    void UniversalInvolutiveBasisContainer::myValidateCohenMacaulay(string fcnName) const
    {
      if(!IamCohenMacaulay())
      {
        CoCoA_ERROR("This ideal is not Cohen Macualay", fcnName);
      }
    }
    ...
    RingElem UniversalInvolutiveBasisContainer::myHilbertPol(RingElem s) const
    {
    ...
    RingElem UniversalInvolutiveBasisContainer::myHilbertSeries(RingElem s) const
    {
    ...
    BigInt UniversalInvolutiveBasisContainer::myHilbertFunc(BigInt s) const
    {
      myInitializeJBMill();
    ...
    {
      if (!firstSyzygyPtr.get())
      {
        myInitializeJBMill();
        firstSyzygyPtr = auto_ptr<FGModule>(new FGModule(jbMillPtr->mySyzygy()));
      }
    ...
    {
      if (!complementaryDecompositionPtr.get())
      {
        myInitializeJBMill();
        complementaryDecompositionPtr = auto_ptr<ComplementaryDecomposition>(new ComplementaryDecomposition(jbMillPtr->myComplementaryDecompositionPolynomial()));
      }
    ...
    vector<RingElem> UniversalInvolutiveBasisContainer::mySocle() const
    {
      if (!soclePtr.get())
      {
        ...
        soclePtr = auto_ptr<vector<RingElem> >(new vector<RingElem>(pbMillPtr->mySocle()));
      }
      return *soclePtr;
    }
