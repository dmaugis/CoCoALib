...
//   else
      ...
      for ( long i=0 ; i<n ; ++i )
      {
        if (exps1[i] >= MaxExp || exps2[i] >= MaxExp)
          CoCoA_ERROR("exponents too big in binomial", "PPs2Binom");
...
//       }
    ...
    void MatKerToBListAndIndices(ConstMatrixView M, BitermList *BL, ints *Indices, ints *Weights)
    {
      ...
      biterm B, MaxDegB; // WARNING: MaxDegB set but never used!!!
      biterms Bs;
      ivec aux_V; 
      int Index, MaxDeg=0;
    ...
    for ( Index=0 ; Index<IndNo ; Index++ )
      ...
      else
      {
        if (std::abs(small_basis[i][Index]) >= MaxExp/weights[Index+1])
          CoCoA_ERROR("exponents too big in binomial", "MatKerToBListAndIndices");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/TmpToric.C,v 1.20 2018/08/06 12:50:43 bigatti Exp $
// $Log: TmpToric.C,v $
