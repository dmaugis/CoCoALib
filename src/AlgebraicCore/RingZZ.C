...
namespace CoCoA
{
  ...
  public:
    ...
    virtual BigInt myFloor(ConstRawPtr rawx) const;                               // identity fn
    virtual BigInt myCeil(ConstRawPtr rawx) const;                                // identity fn
    virtual BigInt myNearestInt(ConstRawPtr rawx) const;                          // identity fn
...
//???    virtual void swap(ideal& other);
  ...
  {
    CoCoA_ERROR(ERR::ShouldNeverGetHere, "RingZZImpl::myRecvTwinFloat");
  }
...
//    if (myIsZero(rawx)) CoCoA_ERROR(ERR::ZeroRingElem, "IsIrred");
...
//2013-06-15    mpz_mul(import(rawlhs), mpzref(quo), mpzref(RecipDen));
  ...
  bool RingZZImpl::IdealImpl::myTestIsMaximal() const
  {
    return myAssignMaximalFlag(!IamZero() && !IamOne() && mpz_probab_prime_p(AsMPZ(myTidyGensValue[0]), 25)); // trust to luck in randomized primality test
  }


  bool RingZZImpl::IdealImpl::myTestIsPrimary() const
  {
    CoCoA_ERROR(ERR::NYI, "myTestIsPrimary ZZ");
    return false;
  }

  bool RingZZImpl::IdealImpl::myTestIsPrime() const
  { return myAssignPrimeFlag(IamZero() || IamMaximal()); }


  bool RingZZImpl::IdealImpl::myTestIsRadical() const
  {
    CoCoA_ERROR(ERR::NYI, "myTestIsRadical ZZ");
    return false;
  }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/RingZZ.C,v 1.30 2018/05/25 09:24:46 abbott Exp $
// $Log: RingZZ.C,v $
