...
//       myR->mySwap(myEntries[i1][j], myEntries[i2][j]);
  ...
  void DenseMatImpl::myDet(RingElem& d) const
  {
    ...
    { d = 1; return; }
    if (n==1)
    { d = myEntry(0,0); return; }
    if (n==2)
    { d = det2x2(ConstMatrixView(this)); return; }
    if (n==3)      
    { d = det3x3(ConstMatrixView(this)); return; }
    if (n==4)      
    { d = det4x4(ConstMatrixView(this)); return; }
    if (n==5)
    { d = det5x5(ConstMatrixView(this)); return; }
    if (IsRingFp(myR))
    { MatrixFp M(ConstMatrixView(this)); d = det(M); return; }
    if (IsZZ(myR))
    {
//d=DetByCRT(ConstMatrixView(this));return;
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/DenseMatrix.C,v 1.39 2018/05/17 15:34:38 bigatti Exp $
// $Log: DenseMatrix.C,v $
