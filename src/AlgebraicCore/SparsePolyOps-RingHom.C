      ...
      for (long ind=0; ind < NumInd; ++ind)
      {
        ...
        if (/*IsPPMonoidOv(myCodomain) && */IsMatrixOrdering(ordering(PPM(S))))
          if (d >= 32749)  // BUG BUG: nasty hack to avoid exp overflow!!!
            CoCoA_ERROR(ERR::ExpTooBig, "ApplySPRCodomain");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-RingHom.C,v 1.5 2018/07/26 15:34:05 abbott Exp $
