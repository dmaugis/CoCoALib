...
//      clog << "------------------------------------------------------"<<std::endl;
  ...
  {                                                 //???
    if (IsZero(h)) CoCoA_ERROR(ERR::NotNonZero, "DistrMPolyClean::myAddMul");
...
//   }
  ...
  {
    for (summand* iter = mySummands; iter != 0/*nullptr*/; iter = iter->myNext)
  ...
  void DistrMPolyClean::myAddMonomial(const DistrMPolyClean& g)
  {
    ...
    while (f_smnd!=0 &&
      ...
      f_smnd = *(f_prev = &f_smnd->myNext);
    if (f_smnd == 0)  { myPushBack(s.relinquish()); return; }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/DistrMPolyClean.C,v 1.25 2018/05/22 14:16:39 abbott Exp $
