...
namespace CoCoA
{
  ...
  void program()
  {
    ...
    GFB.myAddInfo(v);
    cout << "First GCDFreeBasis = " << FactorBase(GFB) << endl << endl;

    // --------------------------------------------
    // Ex.2 Supplying generators one at a time:
    GCDFreeBasis_RingElem GFB2;
    cout << "Generators supplied one at a time..." << endl;
    GFB2.myAddInfo(RingElem(P, "x*y^4*z^9"));
    GFB2.myAddInfo(RingElem(P, "x*y^2*z^3"));
    cout << "Second GCDFreeBasis = " << FactorBase(GFB2) << endl;
  }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/examples/ex-GCDFreeBasis1.C,v 1.1 2018/06/25 12:27:38 abbott Exp $
// $Log: ex-GCDFreeBasis1.C,v $
